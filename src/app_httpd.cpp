...
static esp_err_t counter_stream_handler(httpd_req_t *req) {
    ...
    //                 res = ESP_FAIL;
    //             } else {
    //                 /* convert jpeg to rgb888 format (meaningful pixel values) */
    ...
    //                     Serial.printf("Image of h = %d, w = %d, c = %d\n", image_matrix->h, image_matrix->w, image_matrix->c);
    //                     for (int i = 0; i < image_matrix->h; i++) {
    //                         for (int j = 0; j < image_matrix->w*3; j+=3) {
    //                             /* Print each third pixel */
    //                             Serial.printf("%d ", image_matrix->item[i*image_matrix->w*3 + j]);
    //                         }
    //                         Serial.println();
    //                     }
    //                     Serial.println();
    ...
    //     frame_time /= 1000;
    //     uint32_t avg_frame_time = ra_filter_run(&ra_filter, frame_time);
    //     Serial.printf("MJPG: %uB %ums (%.1ffps), AVG: %ums (%.1ffps), %u+%u+%u+%u=%u\n",
    //         (uint32_t)(_jpg_buf_len),
    //         (uint32_t)frame_time, 1000.0 / (uint32_t)frame_time,
    //         avg_frame_time, 1000.0 / avg_frame_time,
